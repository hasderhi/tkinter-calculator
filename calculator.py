...
try:
    ...
    try:
        ...
        messagebox.showerror("Fatal Error","Fatal Error occured while importing required modules. Program will be canceled.\nErrorcode = 000")
        print("Fatal Error occured while importing required modules. Program will be canceled. Errorcode = 000")
...
try: #Try to initialize the equation array. If this fails, display an error message and exit the program.
    ...
    messagebox.showerror("Fatal Error while initializing the equation variable. Program will be canceled. \nErrorcode = 003")
    print("Fatal Error while initializing the equation variable. Program will be canceled. Errorcode = 003")
    exit()
...
def createRoot():
    ...
    display = Label(display_frame, width=32, height=3, font=("TkDefaultFont", 21), text="Enter calculation")    #Now the code creates a label called 'display'
    ...
    button7 = Button(buttons_frame, width=15, height=5, text="7", font=("TkDefaultFont", 10), command=lambda: createEquation(7))    #On click, it calls a anonymous (lambda) function 'createEquation' with the number of the button 
    button7.grid(row=0, column=0, sticky="nsew")

    button8 = Button(buttons_frame, width=15, height=5, text="8", font=("TkDefaultFont", 10), command=lambda: createEquation(8))
    button8.grid(row=0, column=1, sticky="nsew")

    button9 = Button(buttons_frame, width=15, height=5, text="9", font=("TkDefaultFont", 10), command=lambda: createEquation(9))
    button9.grid(row=0, column=2, sticky="nsew")

    buttonDiv = Button(buttons_frame, width=15, height=5, text="/", font=("TkDefaultFont", 10), command=lambda: createEquation("/"))
    buttonDiv.grid(row=0, column=3, sticky="nsew")


    button4 = Button(buttons_frame, width=15, height=5, text="4", font=("TkDefaultFont", 10), command=lambda: createEquation(4))
    button4.grid(row=1, column=0, sticky="nsew")

    button5 = Button(buttons_frame, width=15, height=5, text="5", font=("TkDefaultFont", 10), command=lambda: createEquation(5))
    button5.grid(row=1, column=1, sticky="nsew")

    button6 = Button(buttons_frame, width=15, height=5, text="6", font=("TkDefaultFont", 10), command=lambda: createEquation(6))
    button6.grid(row=1, column=2, sticky="nsew")

    buttonMult = Button(buttons_frame, width=15, height=5, text="*", font=("TkDefaultFont", 10), command=lambda: createEquation("*"))
    buttonMult.grid(row=1, column=3, sticky="nsew")


    button1 = Button(buttons_frame, width=15, height=5, text="1", font=("TkDefaultFont", 10), command=lambda: createEquation(1))
    button1.grid(row=2, column=0, sticky="nsew")

    button2 = Button(buttons_frame, width=15, height=5, text="2", font=("TkDefaultFont", 10), command=lambda: createEquation(2))
    button2.grid(row=2, column=1, sticky="nsew")

    button3 = Button(buttons_frame, width=15, height=5, text="3", font=("TkDefaultFont", 10), command=lambda: createEquation(3))
    button3.grid(row=2, column=2, sticky="nsew")

    buttonSubt = Button(buttons_frame, width=15, height=5, text="-", font=("TkDefaultFont", 10), command=lambda: createEquation("-"))
    buttonSubt.grid(row=2, column=3, sticky="nsew")


    button0 = Button(buttons_frame, width=15, height=5, text="0", font=("TkDefaultFont", 10), command=lambda: createEquation(0))
    button0.grid(row=3, column=0, sticky="nsew")

    buttonDel = Button(buttons_frame, width=15, height=5, text="DEL", font=("TkDefaultFont", 10), command=lambda: backspace())
    buttonDel.grid(row=3, column=1, sticky="nsew")

    buttonAC = Button(buttons_frame, width=15, height=5, text="AC", font=("TkDefaultFont", 10), command=lambda: clear())
    buttonAC.grid(row=3, column=2, sticky="nsew")

    buttonAdd = Button(buttons_frame, width=15, height=5, text="+", font=("TkDefaultFont", 10), command=lambda: createEquation("+"))
    buttonAdd.grid(row=3, column=3, sticky="nsew")


    frameEqualsButton = Frame(root, width=540, height=100, bg="#808080") #Now it creates a bigger frame for the equals ('=') button.
    frameEqualsButton.place(x=10, y=488)


    buttonEquals = Button(frameEqualsButton, width=31, height=5, text="=", font=("TkDefaultFont", 10), command=calculate)   #On click, this button calls the function 'calculate'
...
def createEquation(num):    #This function is dependant on 'num'. This means, depending on the value the function was called with - for example: createEquation(7) - the function will act differently.
...
def backspace():            #This function deletes the last entry in the array.
    if equation != []:      #Checks if the array is not empty because we cannot pop a empty array.
...
def calculate():            #Main function of the calculation.
    if equation != []:      #If the array is not empty...
        try:    #...it tries...
            total = str(eval("".join(map(str, equation)))) #...to get a result from the given equation.
            ...
            messagebox.showerror("Error", "Error in calculation process occurred.\nErrorcode = 001") #The program shows an error message...
            print("Error in calculation - Error in calculation process occurred. Errorcode = 001")
